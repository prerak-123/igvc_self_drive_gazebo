...
class Stitcher:
    def __init__(self):
        self.homographies = [np.array([[-7.48133125e+00,  1.30233504e+00,  3.52516081e+03],
    ...
    def stitch(self, left_image, front_image, right_image, h, w):
        ...
        translation = np.array([[1,0,(panorama_size[0]-left_image.shape[1])/2],[0,1,(panorama_size[1]-left_image.shape[0])/2],[0,0,1]])
        # stitched_images = []
        for i,image in enumerate(images):
            temp = cv2.warpPerspective(image,np.matmul(translation,self.homographies[i]), panorama_size)
...
class opencv_algo:
    ...
    def __init__(self):
        ...
        front_image_sub = message_filters.Subscriber(center_CamImg_topic,Image)
        ...
        ts = message_filters.TimeSynchronizer([front_image_sub, left_image_sub, right_image_sub], 10)
        ...
        self.task_list = {'Left' : 0, 'Straight' : 1, 'Right' : 2}
        ...
        self.OccupancyGrid_publisher = rospy.Publisher(occgrid_topic, OccupancyGrid, queue_size = 1)
        self.Output_publisher = rospy.Publisher(cv_output_topic, CV, queue_size = 1)
    ...
    def do_ipm(self, image1, **kwargs):
        ...
        h1, w1 = kwargs.get("grid_height", self.Occupancy_Grid.info.height), kwargs.get("grid_width", self.Occupancy_Grid.info.width)
        h2, w2 = self.Cam_Img_height, self.Cam_Img_width
        source = np.float32([[w // 2 - 150, h//2 + 720*.075], [w // 2 + 150, h//2+ 720*.075], [w//2-1280//2-200, h//2+720//2], [w//2+1280//2 + 200, h//2+720//2]])
        destination = np.float32([[w1//2-w2//2, h1//2-h2//2], [w1//2+w2//2, h1//2-h2//2], [w1//2-w2//2, h1//2+h2//2], [w1//2+w2//2, h1//2+h2//2]])
        M = cv2.getPerspectiveTransform(source, destination)
        invM = cv2.getPerspectiveTransform(destination, source)
        warped = cv2.warpPerspective(image1,M,(w1, h1),flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
    ...
    def intersection_handling(self, image):
        ...
        ret, image = cv2.threshold(warped, 245, 256, cv2.THRESH_BINARY)
    ...
    def straight_lane_detection(self, image):
        ...
        for i in range(len(x0)):
            ...
            if int(x0[i]+D*R)>=0 and int(x0[i]+D*R)<w and int(y0[i]+D*m*R)>=0 and int(y0[i]+D*m*R)<h: 
                final_image[int(y0[i] + D*m*R)][int(x0[i] + D*R)] = 1
            if int(x0[i]+2*D*R)>=0 and int(x0[i]+2*D*R)<w and int(y0[i]+2*D*m*R)>=0 and int(y0[i]+2*D*m*R)<h: 
                final_image[int(y0[i] + 2*D*m*R)][int(x0[i] + 2*D*R)] = 1
    ...
    def publish_Occupancy_Grid(self, occ_grid_list):
        ...
        self.Occupancy_Grid.data = np.array([int(value) for value in occ_grid_list])
        self.OccupancyGrid_publisher.publish(self.Occupancy_Grid)

    def publish_CV_Outputs(self, occ_grid, binary_warped, x0, y0, x1, y1, x2, y2):
        ...
        output = CV()
        output.x0 = np.array(x0)
        output.x1 = np.array(x1)
        output.x2 = np.array(x2)
        output.y0 = np.array(y0)
        output.y1 = np.array(y1)
        output.y2 = np.array(y2)

        output.warped = warped 
        output.warped_shape = np.array([1280, 720])
        
        output.occGrid = occGrid
        output.occGrid_shape = np.array([1280, 720])
    ...
    def run_algo(self):
        ...
        if self.stitching:
            image = self.stitcher.stitch(self.left_image,self.front_image,self.right_image, h = 1000, w = 2000)
        elif self.task == 0:
        ...
        if self.task == 0 or self.task == 2:
            (x0, y0, x1, y1, x2, y2), binary_warped = self.intersection_handling(image)
        elif self.task == 1:
            (x0, y0, x1, y1, x2, y2), binary_warped = self.straight_lane_detection(image)
        ...
        if self.task==1:
            current_lane = self.current_lane(x0, y0, x2, y2, binary_warped.shape[1])

        ## Get Lane Occupancy grid from birds eye view
        lane_occ_grid_img = binary_warped

        ## Publish Lane Occupancy grid
        self.publish_CV_Outputs(lane_occ_grid_img, binary_warped, x0, y0, x1, y1, x2, y2)
