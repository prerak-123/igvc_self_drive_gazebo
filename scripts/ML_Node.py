...
class opencv_algo():
    ...
    def run_algo(self):
        ...
        #occGrid
        lane_occ_grid_img_shape = np.shape(lane_occ_grid_img)
        lane_occ_grid_img_shape = list(lane_occ_grid_img_shape)
        
        temp = lane_occ_grid_img.flatten()
        temp = list(temp)
        
        pub_msg.occGrid = temp
        pub_msg.occGrid_shape = lane_occ_grid_img_shape
        
        #warped
        warped_shape = np.shape(warped)
        warped_shape = list(warped_shape)
        
        temp = warped.flatten()
        temp = list(temp)
        
        pub_msg.warped = temp
        pub_msg.warped_shape = warped_shape
